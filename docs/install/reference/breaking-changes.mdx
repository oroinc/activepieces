...
#### The OroCommerce piece signs webhook deliveries by default

The Oro Webhook Event trigger now registers its webhook with a signing secret and discards deliveries whose `Webhook-Signature` header does not match the delivered body. The secret attribute requires OroCommerce 6.1 or newer — on older versions, enabling, re-enabling or republishing a flow with this trigger fails at webhook registration until the trigger's **Sign webhook deliveries** checkbox is turned off. Flows that are already enabled keep running unverified until they are next re-enabled.
...
- Only if a flow uses the Oro Webhook Event trigger against an OroCommerce instance older than 6.1. Untick **Sign webhook deliveries** on the trigger before re-enabling or republishing that flow; existing enabled flows keep working until then.
