# 11. Give the front-end team ownership of the piece

Date: 2026-09-23
Status: accepted
Evidence: a message from the fork owner, 23 Sep 2026; the tracking ticket, 3 Aug 2026 (ownership to be
transferred to the front-end team)
...
The fork owner has handed these to the front-end team that maintains the piece.
...
The front-end team owns everything under `packages/pieces/community/orocommerce/`:

- **The package name.** Record 7 is decided by the front-end team. It no longer waits on the fork owner.
- **The version.** The front-end team applies record 10: decides major, minor or patch for each change,
...
- **Review of every change to that code.** Piece pull requests are cross-reviewed inside the front-end
  team: each one needs approval from a front-end developer other than its author before it merges into
  `poc/orocommerce`. The fork owner is not a required reviewer.

Outside that folder the front-end team owns only the piece sections of `docs/FORK-UPDATE.md` (§2
...
- Record 4's rules are unchanged: piece pull requests still target `poc/orocommerce`, and the image branch
  ...
  branch - record 4 gives that to whoever bumps the piece, which is now the front-end team.
